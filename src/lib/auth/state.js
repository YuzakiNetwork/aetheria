...
import { useMultiFileAuthState } from "@itsliaaa/baileys";
...
class AuthStateManager {
	...
	async getMySQLAuthState() {
		let useMySQLAuthState;

		try {
			const mysqlAuthPackage = "mysql-baileys";
			const mysqlBaileys = await import(mysqlAuthPackage);
			useMySQLAuthState = mysqlBaileys.default;
		} catch (error) {
			throw new Error(
				`MySQL auth backend is unavailable. Reinstall dependencies without --ignore-scripts or use AUTH_STORE=local. ${error.message}`
			);
		}
