...
class PluginManager {
	constructor(botConfig) {
		...
		this.sessionName = BOT_CONFIG.sessionName || "aetheria";
	...
	async loadPlugins() {
		...
		const pluginFolderFilter = this.getPluginFolderFilter();
		...
		try {
			const pluginFolders = readdirSync(pluginsDir, {
				...
				.filter((folder) =>
					pluginFolderFilter ? pluginFolderFilter.has(folder) : true
				);

			print.info(`🌱 Loading plugins from: ${pluginsDir}`);
			print.info(
				`🎮 Enabled plugin folders: ${
					pluginFolderFilter
						? [...pluginFolderFilter].join(", ")
						: "all"
				}`
			);
	...
	getPluginFolderFilter() {
		const rawFolders = process.env.BOT_PLUGIN_FOLDERS || "info,rpg,owner";
		const folders = rawFolders
			.split(",")
			.map((folder) => folder.trim())
			.filter(Boolean);

		if (!folders.length || folders.includes("all")) {
			return null;
		}
	...
	startPeriodicTask(plugin) {
		this.scheduler.startTask(plugin);
	}

	stopPeriodicTask(name) {
		this.scheduler.stopTask(typeof name === "string" ? name : name?.name);
	}
