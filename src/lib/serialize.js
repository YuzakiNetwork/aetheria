 ...
 * @returns {string} Random hexadecimal string with 'AETH' prefix.
 */
const randomId = (length = 32) => {
	const hex = randomBytes(length).toString("hex").toUpperCase();
	return "AETH" + hex.substring(4, length);
};
...
class MessageParser {
	...
	/**
	 * Extract a command-like ID from a native flow response.
	 *
	 * @param {any} msg Parsed message node.
	 * @returns {string}
	 */
	static extractNativeFlowResponseId(msg) {
	...
	static extractBody(msg, message) {
		const nativeFlowId =
			MessageParser.extractNativeFlowResponseId(msg) ||
			MessageParser.extractNativeFlowResponseId(
...
class SerializedMessageBuilder {
	...
	static #attachMethods(m, sock) {
		...
		const _buildMentions = (text, extra = []) => [
			...new Set([
				m.sender,
				...safeParseMention(text),
				...(Array.isArray(extra) ? extra : []),
			]),
		];

		const _buildContentOptions = (options = {}, extraKeys = []) => {
			const sendOptionKeys = new Set([
				"from",
				"quoted",
				"messageId",
				"ephemeralExpiration",
			...
			return Object.fromEntries(
				Object.entries(options).filter(
					([key, value]) =>
						value !== undefined && !sendOptionKeys.has(key)
				)
			);
		};

		const richResponseKeys = [
			"alignment",
			"code",
			"contentText",
			"disclaimerText",
		...
		const hasRichResponseValue = (value) => {
		...
		const getRichFallbackText = (data = {}, options = {}) => {
			const direct = [
				data.headerText,
				data.contentText,
				data.footerText,
			...
			if (Array.isArray(data.richResponse)) {
				const parts = data.richResponse
					.map((item) => item?.text || item?.messageText || "")
					.map((value) => String(value || "").trim())
					.filter(Boolean);
		...
		const _normalizeRichResponsePayload = (data = {}) => {
			const payload = {};

			for (const key of richResponseKeys) {
				if (hasRichResponseValue(data[key])) {
					payload[key] = data[key];
				}
			}

			const needsArrayWrapper =
				!Array.isArray(payload.richResponse) &&
				(payload.inlineImage || payload.items || payload.latex) &&
				!payload.code &&
				!payload.links &&
				!payload.table;

			if (!needsArrayWrapper) {
				return payload;
			}
			...
			if (payload.headerText) {
				richResponse.push({ text: payload.headerText });
				delete payload.headerText;
			}

			if (payload.contentText) {
				richResponse.push({ text: payload.contentText });
				delete payload.contentText;
			}

			if (payload.items) {
				richResponse.push({ items: payload.items });
				delete payload.items;
			}

			if (payload.inlineImage) {
				richResponse.push({
					inlineImage: payload.inlineImage,
					imageText: payload.imageText,
					alignment: payload.alignment,
					tapLinkUrl: payload.tapLinkUrl,
				});
				delete payload.inlineImage;
				delete payload.imageText;
				delete payload.alignment;
				delete payload.tapLinkUrl;
			}

			if (payload.latex) {
				richResponse.push({
					text: payload.contentText || data.contentText || "",
					latex: payload.latex,
				});
			...
			if (payload.footerText) {
				richResponse.push({ text: payload.footerText });
				delete payload.footerText;
			}

			payload.richResponse = richResponse;
		 ...
		 * @returns {Promise<any>}
		 */
		// ponytail: interactive / list / button helpers all currently
		...
		m.reply = async (text, options = {}) => {
			...
			if (!chatId || typeof chatId !== "string") {
				throw new Error("Cannot reply: invalid chat JID.");
			}
		...
		/**
		 * Reply with a classic WhatsApp buttons message.
		 *
		 * @param {string} text Reply text.
		 * @param {Array<{ text?: string, displayText?: string, id?: string, buttonId?: string }>} [buttons=[]] Buttons.
		 * @param {any} [options={}] Send options and content extras.
		 * @returns {Promise<any>}
		 */
		m.replyButtons = async (text, _buttons = [], options = {}) => {
			const text2 = typeof text === "string" ? text : "";
			return m.reply(text2, options);
		};

		/**
		 * Reply with native flow interactive buttons supported by `baileys`.
		 *
		 * @param {string} text Reply text.
		 * @param {Array<any>} [buttons=[]] Native flow buttons.
		 * @param {any} [options={}] Send options and content extras.
		 * @returns {Promise<any>}
		 */
		m.replyInteractive = async (text, _buttons = [], options = {}) => {
			const text2 = typeof text === "string" ? text : "";
			return m.reply(text2, options);
		};

		/**
		 * Reply with a WhatsApp list message in private chat, or native flow sections in groups.
		 *
		 * @param {string} text Reply text.
		 * @param {Array<{ title?: string, rows?: Array<{ title?: string, text?: string, description?: string, id?: string, rowId?: string }> }>} [sections=[]] List sections.
		 * @param {any} [options={}] Send options and content extras.
		 * @returns {Promise<any>}
		 */
		m.replyList = async (text, _sections = [], options = {}) => {
			const text2 = typeof text === "string" ? text : "";
			return m.reply(text2, options);
		};

		/**
		 * Reply with a poll message.
		 *
		 * @param {string} name Poll title.
		 * @param {string[]} values Poll options.
		 * @param {any} [options={}] Send options and poll config.
		 * @returns {Promise<any>}
		 */
		m.replyPoll = async (name, values = [], options = {}) => {
			const pollValues = [
				...new Set(
			...
			if (pollValues.length < 2) {
				return m.reply(String(name), options);
			}
			...
			const poll = {
				...
				selectableCount: Math.max(
					1,
					Math.min(
						Number(options.selectableCount || 1),
						pollValues.length
					)
				),
			};
		...
		/**
		 * Reply with a rich response message.
		 *
		 * @param {{ disclaimerText?: string, headerText?: string, contentText?: string, title?: string, table?: any[], code?: string, language?: string, links?: any[], richResponse?: any[], noHeading?: boolean, footerText?: string, fallbackText?: string }} data Rich response payload.
		 * @param {any} [options={}] Send options.
		 * @returns {Promise<any>}
		 */
		m.replyRichResponse = async (data = {}, options = {}) => {
			const fallbackText = getRichFallbackText(data, options);
			return m.reply(fallbackText || "Rich response kosong.", options);
		};

		/**
		 * Reply with a table message.
		 *
		 * @param {{ disclaimerText?: string, headerText?: string, contentText?: string, title?: string, table?: any[], noHeading?: boolean, footerText?: string }} data Table payload.
		 * @param {any} [options={}] Send options.
		 * @returns {Promise<any>}
		 */
		m.replyTable = async (data = {}, options = {}) => {
			const rows = Array.isArray(data.table) ? data.table : [];

			if (!rows.length) {
				return m.reply(
					data.contentText || data.headerText || "",
					options
				);
			}

			return m.replyRichResponse(
				{
					...
					title: data.title || "",
					table: rows,
					noHeading: Boolean(data.noHeading),
