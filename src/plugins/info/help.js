...
export default {
	...
	execute: async (m, { plugins, isOwner }) => {
		...
		if (m.args.length === 0) {
			...
			listSections = [
					...
					rows: [
						{
							title: "RPG Hub",
							description:
								"Pusat aksi petualangan interaktif Aetheria",
							id: `${m.prefix}rpg`,
						},
						{
							title: "Quest Board",
							description: "Misi harian, mingguan, dan guild",
							id: `${m.prefix}misi`,
						},
						{
							title: "Owner Dashboard",
							description: "Bantuan command owner",
				...
				},
				{
					...
					rows: sortedCategories.map(([category, cmds]) => {
						const categoryMeta = getCategoryMeta(category);

						return {
							title: categoryMeta.label,
							description: `${cmds.length} command tersedia`,
							id: `${m.prefix}help ${category}`,
						};
					}),
				},
			];
		...
		if (listSections && typeof m.replyList === "function") {
			await m.replyList(response.trim(), listSections, {
				buttonText: "Pilih Menu",
				footer: BRAND_CONFIG.name,
				title: `${BRAND_CONFIG.name} Menu`,
			});
			return;
		}
